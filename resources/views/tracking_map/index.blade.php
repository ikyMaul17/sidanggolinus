...
@section('content')
<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Tracking</span> Map</h4>
...
<script>
    $(document).ready(function() {
        ...
        function addBusMarkers(buses, map) {
            ...
            buses.forEach(function(bus) {
                var busMarker = L.marker([bus.latitude, bus.longitude])
                    .addTo(map)
                    .bindPopup('<b>' + bus.nama + '</b><br>' + bus.nama_supir)
                    .openPopup();
        ...
        function fetchAndUpdateMap() {
            // Fetch bus data
            $.get("{{ route('admin_tracking.buses') }}", function(buses) {
                ...
                $.get("{{ route('admin_tracking.stops') }}", function(stops) {
